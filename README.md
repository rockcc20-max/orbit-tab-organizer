# Orbit · Chorme 浏览器标签页聚合工具

一个本地运行的 Chrome 扩展，把多个浏览器窗口里打开的标签页，按网站汇总在同一页。无需账号、服务器或 AI 服务。

## 下载插件

**[下载 Orbit v0.7.1 安装包（ZIP）](https://github.com/rockcc20-max/orbit-tab-organizer/releases/download/v0.7.1/orbit-0.7.1.zip)** · [查看发布版本](https://github.com/rockcc20-max/orbit-tab-organizer/releases)

安装包已编译好，无需安装 Node.js、npm 或自行构建。下载后解压，再按下方的 [Chrome 安装步骤](#安装到-chrome) 加载即可。

请下载名为 `orbit-0.7.1.zip` 的附件，而不是 GitHub 自动提供的 `Source code (zip)` / `Source code (tar.gz)` 源码包。ZIP 版扩展需要通过 Chrome 的“开发者模式”加载，不能双击 ZIP 自动安装。

## 功能

- 网站 Logo、名称和标签页数量一目了然；每个网站下直接列出全部页面，无需展开。
- 根据本机时间显示问候语和日期；紧凑卡片布局适配宽、窄窗口。
- 右上角太阳／月亮图标显示当前明暗，点击直接切换，无需展开菜单；初始跟随浏览器，手动偏好只保存在本机。
- 支持中文、English、한국어、日本語；默认直接显示识别出的本地语言，下拉只列出四种语言，手动选择后会记住。界面、帮助、日期和操作提示同步切换，不改动网页原始标题。
- 右上角可手动同步；同步失败时保留上次成功读取的页面，并提供重试入口。
...
当前版本：`0.7.1`，包含四语言界面、手动同步、易读的网站名称和太阳／月亮一键切换。使用 Manifest V3 和本地编译的 Tailwind CSS。

## 安装到 Chrome

使用上面的已编译安装包即可安装，无需编写代码。

1. [下载安装包](https://github.com/rockcc20-max/orbit-tab-organizer/releases/download/v0.7.1/orbit-0.7.1.zip)，解压到一个准备长期保留的文件夹，不要只打开 ZIP 或双击 `manager.html`。
...
网站 Logo、名称和页面列表集中展示，可以搜索并快速切换页面。右上角直接选择界面语言、点击太阳／月亮切换外观，也可手动同步。
...
- `npm run package`：重新构建，生成可加载的 `dist/Orbit/` 和可分享的 `dist/orbit-0.7.1.zip`。
...
修改 `manager.js`、`utils.js`、`icons.js` 或 `tailwind.input.css` 后需重新构建。`i18n.js` 和 `theme.js` 随扩展本地加载，恢复语言与外观偏好，不调用翻译服务。若 Chrome 加载的是 `dist/Orbit/`，修改后请重新运行 `npm run package`，再到扩展管理页刷新。
...
Lark／飞书文档显示易读名称，Meegle／飞书项目单独显示，OKX 显示品牌名称。不同企业的文档域名仍保留独立分组，卡片下方显示原域名；切换界面语言不会改变分组或重复判断。
...
`tabs` 用于读取标签页信息；`favicon` 用于通过 Chrome 自身接口显示网站图标。0.6.2 已移除未被功能使用的 `storage` 扩展权限；外观与语言偏好保存在本机页面的 `localStorage` 中，不包含标签页信息。完整说明见 [PRIVACY.md](PRIVACY.md)。
